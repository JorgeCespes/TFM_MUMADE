...
# Filtrar los términos específicos y aquellos que contienen "..." o "'" o "’"
top_terms_df <- top_terms_df %>%
  filter(!term %in% c("⁎", "−", "…", "yes", "std", "unstd","’", "swb", "pwb", "almps", "–", "*","httpswwwquoracomwhatsthebestwaytoincreaseproductivity", "httpswwwquoracomunansweredhowdoiprotectemployeesandbusinessesincovid","httpswwwquoracomwhataresomementalhealthtipsduringcovid", "𝖥", "***", "∗∗∗")) %>%
  filter(!grepl("\\.\\.\\.", term)) %>%  # Filtrar términos que contienen "..."
  filter(!grepl("'", term)) %>%  # Filtrar términos que contienen "'"
  filter(!grepl("’", term)) %>%  # Filtrar términos que contienen "’"
  arrange(desc(tfidf)) %>%
...
ggraph(bigram_graph, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Red de bigramas: COVID-19", 
       subtitle = "Top 60")
...
ggraph(trigram_graph, layout = "fr") +
  ...
  geom_node_text(aes(label = name), repel = TRUE, size = 3.25) +
  theme_void() +
  labs(title = "Red de trigramas: COVID-19", 
       subtitle = "Top 60")
...
ggplot(sentiment_totals_df, aes(x = reorder(Sentiment, -Total), y = Total, fill = Sentiment)) +
  ...
  labs(title = "Sentimientos: COVID-19", subtitle = "Quora", x = "Sentimiento", y = "Total") +
  theme_elegante() +
...
additional_stopwords <- c("upvote", "upvotes", "view", "views", "y", "dfollow", "answer", "answers", 
                          ...
                          "cicd", "tool", "bring", "ci", "didn", "t", "k", "don", "ashwani deswal", "completion onthefly analysis")
...
wordcloud(words = names(word_freq), freq = word_freq, min.freq = 200, 

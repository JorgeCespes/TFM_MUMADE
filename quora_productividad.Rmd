...
```{r}
# Find the top terms by TF-IDF excluding the specified terms
top_terms <- sort(colSums(tfidf_matrix), decreasing = TRUE)
top_terms_df <- data.frame(term = names(top_terms), tfidf = top_terms) %>%
  filter(!term %in% c("httpswwwquoracomwhatsthebestwaytoincreaseproductivity", "httpswwwquoracomhowdoesmentalhealthaffectproductivityinanorganisation", "’")) %>%  # Filter out the specified terms
  arrange(desc(tfidf)) %>%
  head(18)
top_terms_df
```
...
top_terms_df <- top_terms_df %>%
  filter(!term %in% c("⁎", "−", "…", "yes", "std", "unstd","’", "swb", "pwb", "almps", "–", "*","httpswwwquoracomwhatsthebestwaytoincreaseproductivity", "𝖥", "***", "∗∗∗")) %>%
...
ggplot(top_trigrams, aes(x = reorder(trigram, n), y = n, fill = reorder(trigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 trigramas", x = "Trigram", y = "Frecuencia") +
  theme_elegante() +                      # Apply a classic theme
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        axis.title.x = element_text(face = "bold"),  # Título del eje x en negrita
        axis.title.y = element_text(face = "bold"),  # Título del eje y en negrita
...
unwanted_words <- c("di", "yr", "doi", "org", "o", "p", "https", "res", 
                    ...
                    "int", "macmillan", "ppi", "yes", "claimant", "count", "iec", "fjmd", "t", "pa", "creed", "univ", "connecticut", "garcíagómez", "sjpsagepubcom", "tue", "sep", "ii", "download", "iza", "e", "ects", "jstor", "ltd", "𝐊", "q", "de", "witte", "copyright", "john", "wiley", "httpsdoiorgjournalpone", "sarti", "zella", "s", "j", "epidemiol", "jung", "dw", "kwak", "kk", "vol", "pp", "plos", "bmc", "quora", "api", "scraper", "im", "album", "yupoo")
...
ggraph(cooccurrence_graph, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Grafo de coocurrencia de términos", 
...
ggplot(sentiment_totals_df, aes(x = reorder(Sentiment, -Total), y = Total, fill = Sentiment)) +
  ...
  labs(title = "Sentiment Totals by Emotion", x = "Emotion", y = "Total", fill = "Emotion") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
# Combine sentiment scores into a data frame
sentiment_df <- data.frame(syuzhet = syuzhet_vector, bing = bing_vector, afinn = afinn_vector, doc_id = 1:length(syuzhet_vector))
...
ggplot(sentiment_melt, aes(x = doc_id, y = score, color = method)) +
  geom_line() +
  labs(title = "Sentiment Scores Over Documents", x = "Document ID", y = "Sentiment Score") +
  theme_minimal()
...
par(mfrow = c(3, 1))
plot(syuzhet_vector, type = "l", main = "Syuzhet Sentiment Trajectory", xlab = "Document ID", ylab = "Sentiment Score")
plot(bing_vector, type = "l", main = "Bing Sentiment Trajectory", xlab = "Document ID", ylab = "Sentiment Score")
plot(afinn_vector, type = "l", main = "Afinn Sentiment Trajectory", xlab = "Document ID", ylab = "Sentiment Score")

```

```{r}
# Create bar plot to compare sentiment between methods
ggplot(sentiment_melt, aes(x = method, y = score, fill = method)) +
  geom_bar(stat = "identity") +
  labs(title = "Comparison of Sentiment Methods", x = "Method", y = "Sentiment Score") +
  theme_classic()
...
```
```{r}
# Define additional stopwords, including terms to be excluded
additional_stopwords <- c("upvote", "upvotes", "view", "views", "y", "dfollow", "answer", "answers", 
                          "jetbrains", "sponsor", "platform", "connect", "pipeline", "support", 
                          "technology", "new", "code", "quality", "idenative", "inspection", 
                          "cicd", "tool", "bring", "ci", "didn", "t", "k", "don")

# Clean the text data with the additional stopwords
clean_corpus <- corpus %>%
  tm_map(content_transformer(tolower)) %>%
  tm_map(removePunctuation) %>%
  tm_map(removeNumbers) %>%
...
ggplot(top_bigrams, aes(x = reorder(bigram, n), y = n, fill = reorder(bigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 Bigrams in Text Corpus", x = "Bigram", y = "Frequency") +
  scale_fill_viridis_d(option = "A") +  # Use a color palette from the viridis package
...
ggplot(top_trigrams, aes(x = reorder(trigram, n), y = n, fill = reorder(trigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 Trigrams in Text Corpus", x = "Trigram", y = "Frequency") +
  scale_fill_viridis_d(option = "A") +  # Use a color palette from the viridis package
  theme_classic() +                      # Apply a classic theme
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")  # Rotate x-axis labels and remove legend
...
# Supongamos que polarity_df tiene dos columnas: polarity y count
# Convertir las polaridades a factores con etiquetas en español
polarity_df$polarity <- factor(polarity_df$polarity, levels = c("positive", "negative"), labels = c("Positiva", "Negativa"))
...
stopwords_custom <- c("sponsored", "jetbrains", "connect", "ci", "tool", "idenative", "cicd", "pipeline")
...
bigram_freq <- bigram_filtered %>%
  count(word1, word2, sort = TRUE) %>%
  filter(n >= 40) # Frecuencia mínima de bigramas
...
# Filtrar términos con coocurrencia baja
cooccurrence_df <- cooccurrence_df %>%
...
set.seed(175)  # Asegura la reproducibilidad
ggraph(cooccurrence_graph, layout = "fr") +
  geom_edge_link(aes(edge_alpha = frequency, edge_width = frequency), color = "grey", show.legend = FALSE) +
  geom_node_point(color = "blue", size = 5) +
  geom_node_text(aes(label = name), repel = TRUE, size = 5) +  # Usar repel para mejorar la distribución de etiquetas
  theme_void() +
  labs(title = "Grafo de coocurrencia de Términos") +
  theme(plot.title = element_text(size = 14, face = "bold"))
```
...
ggplot(sentiment_totals_df, aes(x = reorder(Sentiment, -Total), y = Total, fill = Sentiment)) +
  geom_bar(stat = "identity") +
  labs(title = "Sentimientos: Productivity and mental health", subtitle = "Quora", x = "Sentimiento", y = "Total") +
  theme_elegante() +

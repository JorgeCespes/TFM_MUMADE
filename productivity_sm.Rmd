...
productivity <- lapply(files, pdf_text)
```

```{r}
# Combinar todos los textos en un único vector de caracteres
productivity_text <- unlist(productivity)
```

```{r}
# Crear un corpus de texto a partir del texto extraído
corpus <- Corpus(VectorSource(productivity_text))
```
...
ggplot(top_terms_df, aes(x = reorder(term, tfidf), y = tfidf, fill = term)) +
  ...
  labs(title = "Top 15 términos: Productivity and MH", subtitle = "Literatura en inglés", x = "Término", y = "TF-IDF") +
  theme_elegante() +
...
print(length(productivity_text))  # Verifica la cantidad de documentos
...
productivity_clusters <- data.frame(text = productivity_text, cluster = kmeans_result$cluster)

# Imprimir el resultado del clustering
print(productivity_clusters)
...
word_freq_df <- word_freq_df[word_freq_df$freq >= 70,]
...
ggplot(word_freq_df, aes(label = word, size = freq, color = freq)) +
  ...
  theme_elegante() +
  labs(title = "Productivity and MH: lit.ENG", 
       subtitle = "Mínima frecuencia: 70") +
  theme(
...
bigram_counts_filtered <- bigram_counts %>%
  filter(!bigram %in% c("p p", "https doi", "doi org", "t t", "o o", "be be", "𝐊 𝐊", "iza world"))
...
ggplot(top_bigrams, aes(x = reorder(bigram, n), y = n, fill = reorder(bigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 bigramas: Productivity and MH", subtitle = "Literatura en inglés", x = "Bigrama", y = "Frecuencia") +
  ...
  theme(axis.title.x = element_text(face = "bold", size = 10),   # Pone en negrita el título del eje x y ajusta el tamaño
        axis.title.y = element_text(face = "bold", size = 10),   # Pone en negrita el título del eje y y ajusta el tamaño
        plot.title = element_text(size = 12),  # Ajusta el tamaño del título principal
...
ggplot(top_trigrams, aes(x = reorder(trigram, n), y = n, fill = reorder(trigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 trigramas: Productivity and MH", subtitle = "Literatura en inglés", x = "Trigrama", y = "Frecuencia") +
  theme_elegante() +                      # Aplica el tema elegante
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none") +  # Rota las etiquetas del eje x y elimina la leyenda
  theme(axis.title.x = element_text(face = "bold", size = 10),   # Pone en negrita el título del eje x y ajusta el tamaño
        axis.title.y = element_text(face = "bold", size = 10),   # Pone en negrita el título del eje y y ajusta el tamaño
...
ggraph(bigram_graph, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Productivity and MH: Literatura en inglés", subtitle = "Top 60")
...
unwanted_words <- c("di", "yr", "doi", "org", "o", "p", "https", "res", 
                    "claimant count", "std", "unstd", "be", "garciagómez", 
                    "int", "macmillan", "ppi", "yes", "claimant", "count", "iec", "fjmd", "t", "pa", "creed", "univ", "connecticut", "garcíagómez", "sjpsagepubcom", "tue", "sep", "ii", "download", "iza", "e", "ects", "jstor", "ltd", "𝐊", "q", "de", "witte", "copyright", "john", "wiley", "httpsdoiorgjournalpone", "sarti", "zella", "s", "j", "epidemiol", "jung", "dw", "kwak", "kk", "vol", "pp", "plos", "bmc", "richard", "layard", "ilrsagepubcom", "east", "tennessee", "int", "macmillan", "ppi", "yes", "claimant", "count", "iec", "fjmd", "t", "de", "witte", "j", "plos", "one",
                    "𝐊", "soc", "sci", "download", "ilrsagepubcom", "howard", "univ", "undergrad", "ilrsagepubcom", "httpdoiorgjournalpone", "wiley", "kk", "q", "wiley", "john", "copyright", "sarti", "zella", "ltd", "bubonya", "tennessee", "east", "ilrsagepubcom", "june", "richard", "e", "layard", "n", "joan", "proto", "eugenio", "jt", "january", "wolizaorg")
...
ggraph(bigram_graph, layout = "fr") +
  ...
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  theme_void() +
  labs(title = "Productivity and MH: Literatura en inglés", 
       subtitle = "Top 60")
...
ggraph(cooccurrence_graph, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Productivity and MH: Literatura en inglés", subtitle = "Frecuencia > 500") +

...
comparison.cloud(cloud_tdm, random.order = FALSE,
                 ...
                 title.size = 1.2, max.words = 200, scale = c(4, 0.5), rot.per = 0.4)


```
```{r}
# Convertir la matriz de términos por documento a un dataframe
tdm_df <- as.data.frame(as.table(cloud_tdm))
colnames(tdm_df) <- c("word", "emotion", "frequency")

# Ordenar el dataframe por frecuencia de palabras
tdm_df_sorted <- tdm_df %>%
  arrange(desc(frequency))

# Mostrar las primeras filas del dataframe ordenado
head(tdm_df_sorted, 300)
```
...
tdm <- TermDocumentMatrix(corpus_limpio)
...
chars_to_remove <- c("’", "…", "–", "“", "”", "…", "”", "…", "", "httpswwwlavanguardiacomvidaelparodelargaduracionafectaalasaludmentalsegunestudiodelacaixahtml", "•", "cookie")

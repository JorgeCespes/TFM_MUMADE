...
productividad <- lapply(files, pdf_text)

# Combinar todos los textos en un único vector de caracteres
productividad_text <- unlist(productividad)

# Crear un corpus de texto a partir del texto extraído
corpus <- Corpus(VectorSource(productividad_text))
```
...
top_terms_df <- top_terms_df %>%
  filter(!term %in% c("⁎", "−", "sí", "std", "unstd", "ref", "swb", "pwb", "almps", "–", "*", "𝖥","the", "and", "”", "“", "arbetslivsinstitutet", "núm", "näswall","***", "∗∗∗", "—")) %>%
...
ggplot(top_terms_df, aes(x = reorder(term, tfidf), y = tfidf, fill = term)) +
  ...
  labs(title = "Top 15 términos: Productividad y SM", subtitle = "Literatura en español", x = "Término", y = "TF-IDF") +
  theme_elegante() +
...
productividad_clusters <- data.frame(text = productividad_text, cluster = kmeans_result$cluster)

# Imprimir el resultado del clustering
print(productividad_clusters)
...
wordcloud(words = top_terms_df$term, 
          ...
          max.words = 100, 
          random.order = FALSE, 
          rot.per = 0.1,  # Reducir la cantidad de palabras rotadas
          scale = c(3, 0.5),  # Ajustar la escala
          colors = viridis(100))
```
```{r}
library(ggwordcloud)
...
ggplot(word_freq_df, aes(label = word, size = freq, color = freq)) +
  ...
  theme_elegante() +
  labs(title = "Productividad y salud mental: lit. en ESP", 
...
ggplot(top_bigrams, aes(x = reorder(bigram, n), y = n, fill = reorder(bigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 bigramas: Productividad y SM", subtitle = "Literatura en español",  x = "Bigrama", y = "Frecuencia") +
...
ggplot(top_trigrams, aes(x = reorder(trigram, n), y = n, fill = reorder(trigram, n))) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 20 trigramas: Productividad y SM", subtitle = "Literatura en español", x = "Trigrama", y = "Frecuencia") +
  theme_elegante() +                      # Aplicar el tema elegante
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none") +  # Rotar las etiquetas del eje x y eliminar la leyenda
  theme(axis.title.x = element_text(face = "bold"),   # Poner en negrita el título del eje x
        axis.title.y = element_text(face = "bold"))   # Poner en negrita el título del eje y
...
unwanted_words <- c("di", "yr", "doi", "org", "o", "p", "https", "res", 
                    ...
                    "𝐊", "soc", "sci", "download", "ilrsagepubcom", "howard", "univ", "undergrad", "ilrsagepubcom", "httpdoiorgjournalpone", "wiley", "kk", "q", "wiley", "john", "copyright", "sarti", "zella", "ltd", "bubonya", "tennessee", "east", "ilrsagepubcom", "june", "richard", "e", "layard", "n", "joan", "proto", "eugenio", "toma", "si", "fuente", "propia", "elaboración", "trlgss", "have", "medium", "sa", "lud", "sfconsult", "sfmalestar", "argentino", "of", "siempro", "sfinterr", "sfremedio")
...
# Filter to keep only the top 40 bigrams for visualization
...
ggraph(bigram_graph, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Productividad y SM: literatura en español", 
       subtitle = "Top 40")
...
```{r}
# Función para crear matriz de co-ocurrencia
...
# Filtrar términos con baja co-ocurrencia y eliminar caracteres no deseados
coocurrencia_df <- coocurrencia_df %>%
  filter(frecuencia >500)%>%
  filter(!termino1 %in% c("—", "–", "’", "‘", "−", "⁎", "F", "≥", "∗∗", "“", "”", "’") & 
         !termino2 %in% c("—", "–", "“", "”", "’", "‘", "−", "⁎", "F", "∗∗∗"))
...
ggraph(grafo_coocurrencia, layout = "fr") +
  ...
  theme_void() +
  labs(title = "Grafo de Co-ocurrencia de Términos") +
